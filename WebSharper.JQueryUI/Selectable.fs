...
type Selectable[<JavaScript>] internal () =
    ...
    *****************************************************************)
    ...
    member this.OnSelected(f : JQuery.Event -> Dom.Element -> unit) =
        this.element.OnAfterRender(fun _ ->  this.onSelected f) |> ignore

    /// Event triggered during the select operation,
    /// on each element added to the selection.
    [<JavaScript>]
    member this.OnSelecting(f : JQuery.Event -> Dom.Element -> unit) =
        this.element.OnAfterRender(fun _ -> this.onSelecting f) |> ignore
    ...
    member this.OnUnselected(f : JQuery.Event -> Dom.Element -> unit) =
        this.element.OnAfterRender(fun _ ->  this.onUnselected f) |> ignore

    /// Event triggered during the select operation,
    /// on each element removed from the selection.
    [<JavaScript>]
    member this.OnUnselecting(f : JQuery.Event -> Dom.Element -> unit) =
        this.element.OnAfterRender(fun _ -> this.onUnselecting f) |> ignore
